...
Additionally, you can use this class to convert a raw jiffies value into an accurate datetime timestamp.
'''
...
class LKHZ():
    ...
    def cpu0_offset(self):
        # HRTIMER_BASE_BOOTTIME is index 2 of the clocks but this could change so we search for
        # the ktime_get_boottime identifier, then get the nanosecond offset. cpu0 cannot be taken
        # offline afaik, so we'll use the first instance of boottime (each cpu has a boottime)
        #
    ...
    def jiffies_to_datetime(self, jiffies):
        ...
        ts = datetime.datetime.fromtimestamp(jiffies_as_seconds_since_boot + btime)

        return ts


    def _gv(self):
        _ = subprocess.check_output(['cat','/proc/uptime','/proc/timer_list']).decode().split('\n')
        ...
        # there will be some variance between the times we read these files due to the
        # tiny overhead of file operations. that number should be very small enough to
        # ignore
        
        # /proc/timer_list is in raw jiffies. [almost] everything else in proc is jiffies/HZ
        for __ in _:
            if __.startswith('jiffies:'):
                self.jiffies = int(__.split(': ')[1])
                break

        # account for the -300 seconds that the jiffie counter starts at (and wraps)
        # as a side note; jiffie counter appears to be 64bit but is actually two 32bit
        # operations in the kernel done intelligently because a 64bit memory read is
        # considerably more expensive than a 32bit read
    ...
    def analyze(self):
        print('\x1b[H\x1b[JCONFIG_HZ from /proc/config.gz indicates HZ is set to {}'.format(self.config_hz))
        print('Running constant calculation of HZ, averages span {} elements\n'.format(self._count))
        print('              seconds    calculated                                                    drift since')
        print('  jiffies    since boot    USER_HZ       min          avg          max       variance    prg start'.format(self._count))
        ...
        while True:
            ...
            print('\x1b[4A{:< 12} {:> 10.0f} {:<12.8f} {:<12.8f} {:<12.8f} {:<12.8f} {:>8.8f} {:> 8.8f}'.format(
                self.jiffies, self.since_boot, self.HZ, self._min, self._avg, self._max, self._delta, _drift))
            print('\n\n{:> 8}      {:>12.8f}'.format(_age, self.offset))
            time.sleep(0.02)


if __name__ == '__main__':
    # run forever calculating the CONFIG_HZ value and display values used to obtain it
    hz = LKHZ()
    hz.analyze()
